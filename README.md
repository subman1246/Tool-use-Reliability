...
**The paper itself is in `paper/`** (`paper/main.tex`, compiles to
`paper/main.pdf`; figures generated by `paper/make_aggregate_figures.py`).
`docs/REVIEWER_FEEDBACK_STATUS.md` tracks external review feedback against the
current draft, including what's fixed, what's verified, and what still needs
new experimental runs. `docs/PAPER_DRAFT.md` is an early-stage draft kept for
historical record only, superseded by `paper/main.tex`.
