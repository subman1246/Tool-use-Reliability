...
\hypersetup{
  ...
  pdftitle={Invocation-Level Reliability of Tool-Using Agents},
  pdfauthor={Anonymous}
}
...
\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=1pt,parsep=0pt]
  ...
  \item A controlled error-injection check that isolates the causal effect of state corruption from the confounds present in the observational baseline/free-running contrast, finding the observational conditional-severity estimate is inflated by roughly $2.9\times$ and that severity is confined to the selection channel.
...
Toolformer~\cite{schick2023toolformer}, ReAct~\cite{yao2023react}, and Gorilla~\cite{patil2023gorilla} established tool-augmented agents. ToolLLM~\cite{qin2024toolllm}, StableToolBench~\cite{guo2024stabletoolbench}, and API-Bank~\cite{li2023apibank} emphasize task success or agreement with reference calls. RoTBench~\cite{ye2024rotbench} and MTU-Bench~\cite{wang2025mtubench} separate selection from argument accuracy; FuncBenchGen~\cite{maekawa2025funcbenchgen} generates fresh dependency-graph tasks and reports stale-value failures. Our distinct estimand is the matched change in invocation correctness when gold history is replaced by the same model's history.

Not all agent evaluation uses one canonical trajectory. $\tau$-bench~\cite{yao2024taubench} grades final environment state; AppWorld~\cite{trivedi2024appworld} uses state-based unit tests that permit multiple solutions; ToolSandbox~\cite{lu2024toolsandbox} evaluates intermediate and final milestone DAGs over arbitrary trajectories. BFCL's multi-turn split~\cite{patil2025bfcl} belongs to this group as well, notwithstanding that it distributes one reference call sequence per task: its checker compares final environment state and accepts the reference execution results as an unordered subset of the model's, so both reordering and additional calls pass, and its trajectory-order check (\texttt{method\_invoke\_order\_checker}) is commented out at the call site and by its own docstring inspects method names but not arguments. These methods avoid the unreachable-canonical-target failure but primarily measure outcome or milestone completion, not local post-divergence competence. Section~\ref{sec:central-result} states the narrower condition under which canonical-call scoring becomes scorer-dominated.
...
$L_d$ is descriptive and invocation-pooled; it is not the stepwise quantity in the Markov recurrence below. Calls from one task are dependent, so all reported intervals use paired task-level bootstrap resampling: each of 2{,}000 draws samples $N_d$ tasks with replacement, includes all positions from each selected task in both arms, recomputes $\bar p_d$, $\bar g_d$, and $L_d$ within the replicate, and pairs baseline/free-running observations by task identity. The 89\% level (5.5th and 94.5th percentiles) matches PyMC's default highest-density convention used elsewhere in this work. When a replicate draws only tasks with $\bar p_d=0$, $L_d$ is undefined and that replicate is excluded; this occurred in fewer than 0.1\% of replicates at any depth.
...
We score selection against the tool appropriate for the value in the model's actual state and, for the primary gold-agreement analysis, score arguments against the canonical trajectory. Digit-only JSON strings are coerced to integers, so formatting is not a value error. Errors are syntactic (parse/schema failure) or semantic (executes but is wrong). Both arms use the same within-step retry budget: if a response fails to parse as a valid tool call (malformed JSON, missing required fields), the model is re-prompted with a structured error message up to two additional times; only the final attempt is scored as that step's invocation, and earlier failed attempts are recorded as syntactic errors but do not count toward $\bar p_d$ or $\bar g_d$. If all retries fail, the step is scored as a syntactic error and the free-running arm continues with the last parseable state. This policy is identical in both arms. Conditional-on-state replay replaces the argument target with the argument implied by the model's actual state, making the selection and argument criteria symmetric.
...
\texttt{gpt-oss-20b} completed zero tasks after pilot calls consumed its daily allowance and is excluded from every empirical claim. The identifiers in the original model configuration were already stale before data collection began: the Qwen2.5 and Llama-3.1-instruct generation specified at design time had been retired from the provider prior to the first run. Provider constraints explain the unequal sample sizes but are not treated as scientific findings.
...
2 & 0.383 & 0.350 & 0.087 $[0.022,0.160]$ & 120 \\
4 & 0.427 & 0.208 & 0.514 $[0.431,0.596]$ & 260 \\
...
Clean-context calls score identically under both rules (0.658 vs.\ 0.658), an implementation invariant. Conditional severity is 0.149 $[0.021,0.268]$ for \texttt{llama-3.1-8b} and 0.316 $[0.066,0.503]$ for \texttt{allam-2-7b}. Thus state corruption is associated with genuine loss of local rule application, but far less than the boundary value assigned by canonical gold agreement. Evidence is limited to two models and a weak copy-argument competence test. A controlled error-injection check on \texttt{allam-2-7b} alone finds a smaller causal estimate, $\hat\pi=+0.108$ $[+0.006,+0.215]$ for selection-channel corruption (argument-channel corruption showed no effect), indicating the observational estimate above is inflated by roughly $2.9\times$: the free-running arm lets the model choose which state to corrupt, while the injection arm assigns it, and only the latter isolates a causal effect.
...
\caption{Discrimination, defined as $P(\text{first-listed}\mid\text{even})-P(\text{first-listed}\mid\text{odd})$, per model. Order matches the reliability ranking of Section~\ref{sec:propagation-large} (both computed from the same routing data).}
...
Before spending API budget we validated the full pipeline against six simulated policies with known ground truth, eliminating seven harness artifacts (retry-budget asymmetry, selection errors mis-attributed under poisoning, a contaminated syntactic-share estimator, among others). All seven were discovered and fixed before any real-model data was collected; 14 regression-test modules lock the corrected behavior and pass on the frozen dataset. Two further artifacts were not coding errors, which is what makes them instructive: comparing carried value against \emph{expected argument} rather than \emph{previous gold output} is correct on a copy-argument task and silently wrong under any transform; and the simulated policies recovered by reading ground truth directly, out of band, through a channel no real model has. Correct code, correct configuration, invalid measurement. The second artifact is why the recovery channel was never estimable from observable data in any version of this pipeline. The 81\% parser-failure finding for \texttt{qwen3.6-27b} was discovered during post-collection cache replay; the parser was fixed, cached completions were re-scored (not re-collected), and no completion changed its parsed value (the failure mode was clean breakage, not silent corruption).
...
This study uses synthetic integer-argument tasks, giving exact state transitions at the cost of ecological validity. The strongest empirical findings are carried by two 7--8B models; three stronger models are largely at ceiling. The unreachable-target proposition is general only through its explicit $\epsilon_k$ condition, while the empirical claim that our generator makes $\epsilon_k$ negligible depends on its hidden-state construction. Validation on natural-language arguments and stateful environments such as ToolSandbox or AppWorld is required before claiming prevalence in deployed agents. We began that validation on BFCL's multi-turn split and record one structural obstacle that is a property of the remedy rather than of any benchmark: conditional-on-state scoring needs a rule mapping the state an agent actually holds to the call that would be correct from it, and a task whose gold trajectory encodes a user's intent rather than a decision rule supplies no such mapping. Section~\ref{sec:remedy} therefore does not transfer to that setting without an additional model of intent, and we do not report conditional severity there.

Three designed conditions were not executed: transformed arguments, randomized presentation order, and a calling-mode ablation. A linear-task control was executed on \texttt{allam-2-7b} alone: at depth 6, $L_6=0.156$ $[0.089,0.229]$ against $0.684$ $[0.624,0.745]$ on routing. The linear variant removes selection dependency but retains value copying; the residual loss is substantially smaller than routing's but excludes zero at depths 4 and 6, indicating that the baseline/free-running contrast captures effects beyond selection-error propagation alone. A recovery-enabled variant that restores the canonical state at a fixed step was also run on \texttt{allam-2-7b} alone (Appendix~\ref{app:repair}): at depth 6 the restoration raised downstream gold agreement by $+0.126$ $[+0.063,+0.198]$ but raised completion of the original task by only $+0.054$ $[+0.000,+0.108]$, an interval including zero and resting on two events, so returning to the canonical trajectory and recovering the original goal are dissociated on this measure even though only the first difference is supported at this sample size. These are material threats, not cosmetic extensions. In particular, \texttt{allam-2-7b}'s anti-correlation may depend on the parity/order presentation, and copy arguments do not test semantic argument construction. Three of the five original models (\texttt{llama-3.1-8b-instant}, \texttt{llama-3.3-70b-versatile}, \texttt{qwen3.6-27b}) were retired from the hosting provider during this study, preventing new ablation conditions from being compared against their primary-arm baselines. Counting the earlier undocumented TPD caps (Section~\ref{sec:data}), this is the fourth distinct instance of unannounced infrastructure instability encountered in this study. A local-quantized replication of \texttt{llama-3.1-8b} was accordingly tested for equivalence against its frozen provider-hosted arm on matched tasks, under a rule fixed before those data existed: agreement within $0.05$ and an 89\% paired interval containing zero, at every shared depth and in both run modes. It failed in five of ten cells (depths 6 and 8 under teacher forcing; depths 4, 6 and 8 free-running), and the local arm exceeded the frozen arm in all ten cells with the gap widening in depth, indicating a systematic offset rather than sampling variation; pooling the two would have moved $L_6$ from $0.687$ to $0.518$. They are therefore reported as separate conditions and never compared cell-for-cell. The cause cannot be isolated, because that retirement preceded the comparison: quantization and the provider's serving stack are confounded, and no further probe of the frozen arm is possible. The unequal nested sample sizes preclude scale or family comparisons. The reported intervals require the task-level artifact for reproduction; call-level binomial intervals would be invalid because steps within a task are dependent.
...
Table~\ref{tab:aggregate-counts} reports the aggregate correct/total counts underlying Table~\ref{tab:depth-results}, together with absolute excess error $A_d$ and error amplification $E_d$. Task-cluster confidence intervals require the per-task records in the release artifact; the deterministic summaries below do not.
...
\section{Linear-task control arm}
\label{app:linear}

The linear variant announces the tool order in the prompt, so the correct tool at each
step is fixed in advance and cannot depend on the value carried in. What it retains is
argument-value copying: each argument is a verbatim copy of the previous observation, so a
wrong value still propagates forward. The arm was executed on \texttt{allam-2-7b} at the
routing arm's depths and sample sizes. Depth 6 stopped on an exhausted daily token
allowance at 31 of 40 planned tasks; the achieved counts are a nested prefix and are used
as such.

\begin{table}[h]
\centering
\caption{Invocation-pooled depth estimands for \texttt{allam-2-7b} on both task families.
Intervals are 89\% paired bootstrap intervals over tasks, the same estimator and
convention used for the routing arm.}
\label{tab:linear-control}
\footnotesize
\begin{tabular}{@{}lrrlr@{}}
\toprule
\multicolumn{5}{c}{Linear (selection dependency removed)} \\
\midrule
depth $d$ & $\bar p_d$ & $\bar g_d$ & $L_d$ [89\% interval] & calls \\
\midrule
1 & 0.950 & 0.950 & \textbf{0.000} $[0.000,0.000]$ & 60 \\
2 & 0.992 & 0.983 & 0.008 $[0.000,0.026]$ & 120 \\
4 & 0.962 & 0.908 & \textbf{0.056} $[0.028,0.086]$ & 260 \\
6 & 0.930 & 0.785 & \textbf{0.156} $[0.089,0.229]$ & 186 \\
\midrule
\multicolumn{5}{c}{Routing (selection dependency present)} \\
\midrule
1 & 0.533 & 0.533 & \textbf{0.000} $[0.000,0.000]$ & 60 \\
2 & 0.383 & 0.350 & 0.087 $[0.022,0.160]$ & 120 \\
4 & 0.427 & 0.208 & 0.514 $[0.431,0.596]$ & 260 \\
...
Linear tasks remove the selection dependency but retain argument-value copying, so the two
arms differ in whether an upstream error can change which tool is correct downstream, not
in whether a wrong value is carried forward. At depth 6 the routing loss is $4.4\times$ the
linear one ($0.684$ against $0.156$), which is consistent with selection-error propagation
accounting for the majority of total loss. The linear intervals at depths 4 and 6 both
exclude zero, consistent with the baseline/free-running contrast capturing effects
beyond selection-error propagation, though the specific mechanism is not resolved by
this control alone; the linear arm is therefore a bound on the selection dependency's
contribution rather than a null. The two families share the harness, the scorer and the
prompt length at each depth, so neither of those explains the gap between them.

\section{Error-injection ablation}
\label{app:injection}

To test whether the observational propagation estimate reflects a causal effect of state
corruption rather than an arm-construction confound, we injected controlled errors into
otherwise identical trajectories for \texttt{allam-2-7b} (52 of 60 planned tasks, 316
pairs). At a specified position $j\in\{1,3,5\}$, two branches diverge from the same
prefix: a \emph{corrupted} branch where one tool output is replaced with a wrong value,
and a \emph{corrected} branch where the original value is preserved. Both branches then
continue through the same model. Because both share an identical prefix and differ in
exactly one message, the contrast isolates state corruption from prompt-style,
retry-history, and demonstration-quality differences.

\paragraph{Validity check.} The corrected branch reproduces the baseline arm's accuracy
exactly across all 40 overlapping tasks at every position tested ($+0.000$ at $j=1,3,5$),
confirming the injection mechanism introduces no construction artifact.

\paragraph{Two injection modes.} \texttt{parity\_flip} changes the carried value's parity,
altering which tool the routing rule selects (a selection-channel corruption).
\texttt{parity\_preserving} changes the value without changing its parity, so the correct
tool is unchanged but the argument is wrong (an argument-channel corruption).

\begin{table}[h]
\centering
\caption{Causal severity from controlled error injection on \texttt{allam-2-7b}, scored
conditional-on-state. Intervals are 89\% paired bootstrap intervals resampling pairs.
Discordant counts are the pairs in which exactly one branch was correct: $b$ favours the
corrected branch, $c$ the corrupted one.}
\label{tab:injection}
\footnotesize
\begin{tabular}{@{}lrrlr@{}}
\toprule
condition & corrected & corrupted & $\hat\pi$ [89\% interval] & $b/c$ \\
\midrule
\texttt{parity\_flip} (selection) & 0.411 & 0.304 & \textbf{$+0.108$} $[+0.006,+0.215]$ & 65/48 \\
\texttt{parity\_preserving} (argument) & 0.411 & 0.443 & $-0.032$ $[-0.057,-0.013]$ & 0/5 \\
\midrule
observational (routing free arm) & n/a & n/a & $+0.316$ $[+0.066,+0.503]$ & n/a \\
\bottomrule
\end{tabular}
\end{table}

Three findings follow. First, the observational estimate is biased upward by
approximately $2.9\times$ ($+0.316$ against $+0.108$), and the observational value lies
outside the causal interval. In the free-running arm the model selects its own
corruptions, so whatever produced the upstream error remains present downstream; the
injected contrast removes that dependence by assigning corruption externally. Second, the
effect is confined to the selection channel: \texttt{parity\_preserving} corruption
produced no degradation at any position, with zero pairs favouring the corrected branch,
so a wrong argument value alone does not measurably impair this model's next call. Third,
severity is not constant along the chain. The position profile is $+0.132$ at $j=1$,
$+0.226$ at $j=3$ and $-0.038$ at $j=5$, of which only $j=3$ excludes zero. The step
model of Section~\ref{sec:propagation-model} treats $\pi$ as position-independent, and
these estimates do not support that assumption.

These results are from a single model at one depth and a truncated allocation. The pooled
selection-channel interval only narrowly excludes zero and one of three positions carries
the effect, so the ablation establishes the direction of the observational bias more
firmly than it establishes the magnitude of the causal severity. The eight tasks not
collected would not change that. We report it as a single-model causal check on the
estimand, not as suite-wide validation.

\section{Recovery-enabled task variant}
\label{app:repair}

Section~\ref{sec:central-result} shows that after a divergence the canonical target is
unreachable, so gold agreement cannot express recovery; Section~\ref{sec:remedy} scores
calls conditional on the state actually held instead. Neither distinguishes an agent that
continues competently from a wrong state from one that returns to the right one, because
the routing task offers no route back. This variant adds one.

\paragraph{Mechanism.} A single additional tool, \texttt{check\_state}, returns the
canonical reference value for the step the agent is on. It consumes a call, does not
advance the task, and is usable once per task. The one-shot budget makes usage binary, and
excludes the degenerate policy of resynchronising before every step, which would convert
the free-running arm into the teacher-forced arm. After a successful repair the agent again
holds the canonical value and can rejoin the gold trajectory, which is what makes genuine
recovery possible rather than merely tidy continuation.

\paragraph{Elected repair measures prompt position, not state.} Two pilots on
\texttt{allam-2-7b} (16 tasks each, 8 at depth 4 and 8 at depth 6, identical seeds and gold
trajectories) offered the tool and let the model choose when to use it. With the budget
rule stated as the final instruction before the first step, the model called
\texttt{check\_state} in 14 of 16 tasks and in every case at step 0, where the held value
equals the canonical value by construction, so each call returned the number already in the
prompt. With the same rule stated inside the task description and the output format stated
last, it called the tool in 0 of 16. No repair landed on a diverged step under either
prompt. The mechanism was not at fault: an always-repair oracle recovers the original
trajectory in 100\% of tasks, and a never-repair policy is step-for-step identical to the
free-running arm. The task supplies no divergence signal, so an agent's only coherent
policies are to ask always or never, and prompt salience selects between them. Raising the
budget would select the former and measure nothing. We therefore assign the repair rather
than eliciting it.

\paragraph{Construction.} As in Appendix~\ref{app:injection}, a value is corrupted at a
fixed position ($j=1$, \texttt{parity\_flip}). The model then runs free, so the trajectory
diverges under its own behaviour. At a fixed later position ($r=3$) both branches receive
the same \texttt{check\_state} exchange: the repaired branch is handed the canonical value,
the unrepaired branch the value it is already carrying, so its repair is a no-op by
construction and the two message histories differ in exactly one number. The segment before
the fork is executed once and inherited by both branches, so the pair is identical by
construction rather than by reproducibility. Both branches then run free to the end of the
task.

\paragraph{Scoring is at completion, not at the repaired call.} Appendix~\ref{app:injection}
already establishes that restoring the true value repairs the \emph{next} call ($+0.000$
against baseline at $j=1,3,5$); re-measuring that would report a known result. The verdict
here is \texttt{completed\_original}: the branch ran every remaining step \emph{and}
finished holding the task's true final output. The constraint $r \le d-2$ is enforced in the
harness so that at least two steps remain to be scored, which is what makes the verdict a
trajectory property. A validity test fixes the distinction: a policy that makes the
repaired call correctly and then drifts is scored as not recovered, where a
next-call-only check would score it a success.

\begin{table}[h]
\centering
\caption{Assigned repair at depth 6 on \texttt{allam-2-7b}, $n=37$ pairs. Intervals are
89\% paired bootstrap intervals resampling pairs; the two branches share an inherited
prefix, so the pair is the independent unit. Downstream rates cover the steps from the
repair onward. Discordant counts are pairs in which exactly one branch was correct: $b$
favours the repaired branch, $c$ the unrepaired one.}
\label{tab:repair}
\footnotesize
\begin{tabular}{@{}lrrlr@{}}
\toprule
measure & repaired & unrepaired & difference [89\% interval] & $b/c$ \\
\midrule
\texttt{completed\_original} & 0.054 & 0.000 & $+0.054$ $[+0.000,+0.108]$ & 2/0 \\
downstream gold agreement & 0.126 & 0.000 & \textbf{$+0.126$} $[+0.063,+0.198]$ & 9/0 \\
downstream conditional-on-state & 0.342 & 0.297 & $+0.045$ $[-0.018,+0.108]$ & 6/3 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result.} Restoring the canonical state raises downstream gold agreement by
$+0.126$ $[+0.063,+0.198]$, an interval excluding zero: the repair demonstrably returns the
model to the canonical path. It does not measurably raise completion of the original task.
That difference is $+0.054$ $[+0.000,+0.108]$, rests on two events in 37 pairs, and its
interval includes zero, so we do not report it as an effect. Conditional-on-state
competence is unchanged by the repair ($+0.045$ $[-0.018,+0.108]$), as expected: the model
continues about as competently from a repaired state as from a corrupted one. The
dissociation is the result. Returning to the canonical state and recovering the original
goal are separable properties, and only the first is purchasable by handing the state back.
We state the asymmetry carefully: the evidence for the first is statistically supported at
this $n$, the absence of the second is not, and a larger sample could move completion off
the floor without disturbing the ordering.

The unrepaired branch completes the original task in 0 of 37 pairs, so the contrast is
attributable to the repair rather than to a corrupted trajectory rejoining gold by
coincidence. An oracle applying the routing rule to whatever value it holds completes 12 of
12 repaired branches and 0 of 12 unrepaired ones, so the measure is not penalising correct
behaviour.

A depth-8 cell was allocated and was still collecting when this version was
prepared; it is not reported here. We state that rather than including a partial
cell, because at the handful of pairs collected the completion measure is a count of
single-digit events and an interval computed from it would convey precision the data
does not carry. The depth-6 cell above is complete at its allocated $n$.

These results are from a single model at one depth. As with
Appendix~\ref{app:injection}, we report them as a single-model check on whether the
distinction is measurable at all, not as suite-wide validation of its magnitude.
