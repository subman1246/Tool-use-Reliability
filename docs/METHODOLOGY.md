...
**Amended after measurement.** The binding constraint turned out to be neither money nor requests but the per-model daily *token* allowance, which is undocumented, non-uniform across models (a 5× spread), and readable only from the body of the HTTP 429 that enforces it. Routing-task cost is roughly quadratic in depth, since the schema grows with depth and the whole prompt is re-sent at every step, so the deepest bin dominates the bill. The originally planned sweep came to 2.9M tokens per model, i.e. 37 days against the binding model. See `docs/METHOD_NOTES_real_run.md` for the measured table.

Two design changes follow, both recorded because they change what the study can claim:

- **Nested unequal n.** Models no longer run identical task sets; they run nested *prefixes* of one suite, sized per model to its own measured allowance. Task identity depends only on `(depth, k, base_seed)` and never on how many tasks were requested, so a smaller allocation is exactly the first tasks of a larger one. Cross-model contrasts are computed on the common prefix and stay exactly paired; each model's own estimates use its full n; the binomial likelihood already accommodates unequal trial counts, and partial pooling shrinks the low-n models further toward their family mean, which is correct rather than a workaround. The cost is unequal precision across models, concentrated on the model with the smallest allowance — which is retained anyway because it is the only dense scale-within-family contrast in the suite.
- **H4 becomes a suite-level claim.** Per-step error counts cannot support a per-model trend at any allocation this budget permits: an exhaustive search over 47,068 equal-n allocations under a fixed invocation ceiling found none with a better worst-case usable-step prefix, because per-step coverage and cost both scale with depth. Pooled across models the counts do clear the threshold, so H4 is tested pooled, with per-model rank correlations shown as directional evidence only. If the achieved pooled counts fail to clear it, H4 is withdrawn rather than reported — that check is in `scripts/report_real.py`, not left to judgement at write-up time.
...
- Unequal precision across models: an artifact of the nested design, not of the phenomenon. Per-model interval widths differ by up to a factor of five because per-model n does. Any statement comparing two models is therefore made on their common task prefix, and any per-model statement carries its own interval. What must not be done — and is not done — is reading a scale or family contrast off point estimates whose precision differs by that much.
- Validation coverage: a simulator validates the channels it exercises and silently certifies the ones it does not. Nine defects in this pipeline were all instances of that single failure mode, three of them invisible to every assertion the suite made, because the suite asserted on rates the simulator itself produced. Two of the nine were not coding errors at all: comparing a carried value against an expected argument is correct on a copy-argument task and only wrong on one that can distinguish them, and the simulator's recovery did exactly what it was configured to do -- through a channel no real model has, which is why the recovery parameters were never estimable from observable data in any version of this study. Mitigations now in place: tests assert on the message list sent to the provider rather than on backend behaviour, and every quantity the analysis derives is checked against a configured ground truth *through the full pipeline* rather than against the generator that produced it.
