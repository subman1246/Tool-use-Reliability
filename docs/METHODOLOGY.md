...
- Validation coverage: a simulator validates the channels it exercises and silently certifies the ones it does not. Seven defects in this pipeline were all instances of that single failure mode, three of them invisible to every assertion the suite made, because the suite asserted on rates the simulator itself produced. Mitigations now in place: tests assert on the message list sent to the provider rather than on backend behaviour, and every quantity the analysis derives is checked against a configured ground truth *through the full pipeline* rather than against the generator that produced it.
